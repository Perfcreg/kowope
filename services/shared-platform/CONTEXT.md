...
- **No Country/Region claim at all** (RFP User Stories 2, 12) — `reference-data-config`'s Country model now exists (ADR-0021), so there's something real to source a value from, but `authentication-service`'s own `User` domain still has no Country field and the admin panel has no way to set one; closing this is `authentication-service`'s own future work now, not blocked on another context anymore. See ADR-0017, ADR-0021.
