package com.uba.mbp.integration.excelimport.notification;
 ...
 * replacing {@code LoggingNotificationClient} now that it exists — the same
 * Camel {@link ProducerTemplate} outbound-HTTP pattern every other adapter
 * in this context uses (ADR-0011), now needed here for the first time (this
 * adapter previously made no outbound HTTP calls of its own — see the
 * added {@code camel-http-starter} dependency). A failed call here falls
 * back to the same local ERROR log the old stand-in always did, never
 * rethrows: an alert that itself fails to send must not become a new
