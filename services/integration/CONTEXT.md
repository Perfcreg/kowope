...
**ADR-0013**: `DetectedWriteOffStore`'s in-memory dedup state is a known, accepted-for-now limitation (same pattern as the ICAD adapter's `PendingClearanceStore`, ADR-0015) — a restart can cause one round of duplicate re-detection, which `memo-balance`'s own consumer-side dedup (Ticket 02) absorbs safely rather than corrupting state.
