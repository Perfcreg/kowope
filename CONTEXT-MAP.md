...
✅ = implemented, 🔶 = partially implemented. `memo-balance` (Tickets 01–08, PR #10) is the first; its `CONTEXT.md` now exists for real, including the published/consumed event schemas other contexts should build against instead of reading its Java source. `integration`'s four adapters are each built on their own unmerged branch — 🔶 until they're merged into `main` together. See [services/integration/CONTEXT.md](services/integration/CONTEXT.md).
