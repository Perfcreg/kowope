...
This repo is multi-context. Each row below is a bounded context; its `CONTEXT.md` is created lazily (on the first resolved term) at the path shown — most don't exist yet. System-wide decisions that don't belong to any single context live in root [docs/adr/](docs/adr/).
...
**Still open**: production deployment target (ADR-0008), mobile channel approach (ADR-0010), GitHub remote not yet created.
