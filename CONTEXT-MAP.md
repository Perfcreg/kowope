...
✅ = implemented, 🔶 = partially implemented. `memo-balance` (Tickets 01–08, PR #10), `integration` (all four adapters: write-off-detection-service, vision-etl-connector, excel-import-service, icad-integration-adapter), and `shared-platform` (authentication-service and notification-service) are all fully implemented. The first three `integration` adapters use Apache Camel — ADR-0011 — against a Fineract stand-in for Finacle and Vision respectively (ADR-0012/ADR-0013); icad-integration-adapter has a materially weaker verification story since no real ICAD sandbox exists (ADR-0014) — see [services/integration/CONTEXT.md](services/integration/CONTEXT.md). `shared-platform`'s authentication-service issues/validates RBAC tokens via MFA-gated login (ADR-0017) plus an admin-gated user-management control panel (ADR-0018), closing the dev-secret interim seam every other implemented service's `SecurityConfig` had documented; its notification-service (ADR-0019) is the single real email-alerting seam every `integration` adapter's `NotificationClient` now calls for real, triggered both synchronously (any context's own `POST /notifications`) and by two real Kafka events (`memo-balance`'s liquidation, `icad-integration-adapter`'s clearance discrepancies). `icad-integration-adapter` also escalates a still-`PENDING` clearance to `CREDIT_ADMIN` once it's older than a configured SLA (ADR-0020) — the one action-plan type (of RFP §3.9's three named examples) with a real pending/resolvable lifecycle to escalate from today. `reference-data-config` (ADR-0021) now serves real, versioned Country/GL-mapping data — `memo-balance`'s `CountryConfigLookup` interim seam is closed for real — but stays 🔶 since date format, holiday calendar, the admin UI, and the core-banking inflow interface (all named in RFP §3.14) remain unbuilt, with no anticipating code anywhere yet. Each implemented context's `CONTEXT.md` exists for real, including published/consumed event schemas other contexts should build against instead of reading its Java source.
...
- **`memo-balance` → `reference-data-config`**: a synchronous REST read (`GET /countries/{code}`, not an event) on every detection ingest, to resolve the account's Country's base currency and GL write-off/recovery codes (ADR-0021).
...
| [0019](docs/adr/0019-notification-service-recipient-resolution.md) | notification-service resolves recipients via static configured groups; its REST endpoint is internal-only and unauthenticated; two real Kafka events are wired as triggers |
| [0020](docs/adr/0020-icad-overdue-clearance-escalation.md) | Overdue ICAD clearances (not a generic action-item abstraction) escalate to CREDIT_ADMIN via notification-service once past a configured SLA |
| [0021](docs/adr/0021-reference-data-config-versioned-country-mapping.md) | reference-data-config: versioned Country/GL mapping (close-and-insert, never mutate in place); its own Postgres schema; unauthenticated internal read API |
